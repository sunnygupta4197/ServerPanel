...
describe('ServerPanel Pro API Tests', () => {
  ...
  beforeAll(async () => {
    ...
    // The constructor already kicks off migrations (this.dbInitPromise) —
    // await that instead of calling migrate.latest() again ourselves, which
    // races the constructor's own call on the single shared :memory: connection.
    await appInstance.dbInitPromise;
  ...
  afterAll(async () => {
    ...
    // Not destroying `database` here — it's a process-wide singleton
    // (require('../src/config/database')) shared with the 'Integration
    // Tests' describe block later in this file; destroying it here would
    // tear down their connection too. The last describe block closes it.
  });
  ...
  describe('Authentication', () => {
    ...
    describe('POST /api/auth/logout', () => {
      test('should logout successfully', async () => {
        // Logout blacklists whatever token is passed — use a disposable
        // token (same payload as adminToken) so the shared adminToken
        // used by every later test in this file stays valid.
        const { id, username, role } = jwt.decode(adminToken);
        const disposableToken = jwt.sign(
          { id, username, role },
          process.env.JWT_SECRET || 'test-secret',
          { expiresIn: '1h' }
        );
  ...
  describe('System Management', () => {
    ...
    describe('POST /api/system/services/:name/:action', () => {
      ...
      test('should reject service names with shell metacharacters', async () => {
        const response = await request(app)
          .post('/api/system/services/nginx%3Brm%20-rf%20%2F/start')
          .set('Authorization', `Bearer ${adminToken}`)
          .expect(400);
    ...
    describe('POST /api/system/execute', () => {
      test('should not exist — removed as an unauthenticated-shell-exec risk', async () => {
        const response = await request(app)
  ...
  describe('Performance', () => {
    test('should respond quickly to health checks', async () => {
      ...
      // /health now does a real DB round-trip (SELECT 1) so it actually
      // reflects readiness, not just process liveness — 100ms was calibrated
      // for a check with zero I/O.
...
describe('Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    adminToken = authCookie ? authCookie.split(';')[0].split('=')[1] : undefined;
  });
